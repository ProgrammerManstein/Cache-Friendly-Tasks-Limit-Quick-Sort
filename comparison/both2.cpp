...
void quickSort_parallel_internal(int* array, int left, int right, int cutoff, int is_task, int depth)
{
	...
	if (i - left > cache_size || (lock == 1 && (i - left > cutoff && task_num < 2 * numThreads) && depth < depth_limit)) {
		depth++;
#pragma omp atomic
		task_num++;
#pragma omp task	
	...
	if (right - i - 1 > cache_size || (lock == 1 && (right - i - 1 > cutoff && task_num < 2 * numThreads) && depth < depth_limit)) {
#pragma omp atomic
		task_num++;
#pragma omp task	
		{
